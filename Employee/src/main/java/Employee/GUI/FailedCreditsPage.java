...
public class FailedCreditsPage extends SceneController {
    @FXML
    ...
    public void executeAnAction(ActionEvent event) throws IOException {
        ...
        } else if (event.getSource() == reactiveCredit && currentCreditId != null) {
            var msg = sendToServerWithResponse("CREDIT,REACTIVE," + currentCreditId);
            if(isResponseValid(msg)){
                showInfo("REACTIVATED", "Credit reactivated successfully");
                openEmployeePage(event, employee);
            }
        }
    }
