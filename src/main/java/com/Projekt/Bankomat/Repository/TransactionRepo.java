...
public interface TransactionRepo extends JpaRepository<Transaction, String> {
    ...
    @Query(value = "SELECT t FROM _TRANSACTION t WHERE t.toAccountNr =?1 and t.isValid = FALSE")
    ...
    @Query(value = "SELECT t FROM _TRANSACTION t WHERE t.toAccountNr = ?1")
    List<Transaction> findAllTransactionsFromBankAccount(String zKonta);

    @Query(value = "SELECT t FROM _TRANSACTION t WHERE t.toAccountNr in " +
    ...
    @Query(value = "SELECT t FROM _TRANSACTION t WHERE t.isValid = TRUE and t.fromAccountNr in " +
            "(SELECT k.accountNr from BANK_ACCOUNT k JOIN user u WHERE u.email = ?1)")
